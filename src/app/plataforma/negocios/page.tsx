...
export default function PlatformBusinessesPage() {
  ...
  return <>
    ...
    {invite && <ModalShell titulo="Negocio creado" onClose={() => setInvite(null)}><div className="w-full max-w-lg rounded-3xl bg-white p-6"><h2 className="text-xl font-black">Negocio creado</h2><p className="mt-2 text-sm text-[#736f83]">Copia este enlace de invitación y envíalo al dueño (expira, es de un solo uso):</p><input readOnly value={invite} className="mt-3 w-full rounded-xl border bg-[#f7f6fa] p-3 text-xs" onFocus={e => e.target.select()} /><button onClick={() => setInvite(null)} className="mt-5 w-full rounded-xl bg-[#5b3df5] py-3 font-bold text-white">Listo</button></div></ModalShell>}
