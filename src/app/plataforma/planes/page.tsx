...
export default function PlatformPlansPage() {
  ...
  return <>
    ...
    {show && <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4"><form onSubmit={createPlan} className="w-full max-w-md rounded-3xl bg-white p-6"><h2 className="text-xl font-black">Nuevo plan</h2><div className="mt-4 grid gap-3"><label className="text-sm font-semibold">Código<input name="code" required className="mt-2 w-full rounded-xl border p-3 uppercase" /></label><label className="text-sm font-semibold">Nombre<input name="name" required className="mt-2 w-full rounded-xl border p-3" /></label><label className="text-sm font-semibold">Máximo de profesionales (vacío = ilimitado)<input name="max" type="number" min="1" className="mt-2 w-full rounded-xl border p-3" /></label><label className="text-sm font-semibold">Precio<input name="price" type="number" min="0" defaultValue={0} className="mt-2 w-full rounded-xl border p-3" /></label></div><div className="mt-5 flex gap-2"><button type="button" onClick={() => setShow(false)} className="flex-1 rounded-xl border py-2.5 font-bold">Cancelar</button><button className="flex-1 rounded-xl bg-[#5b3df5] py-2.5 font-bold text-white">Crear</button></div></form></div>}
