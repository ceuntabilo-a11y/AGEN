...
import { Eye, Mail, MessageCircle, Pencil, Plus, UsersRound } from 'lucide-react'
import { useCallback, useEffect, useState } from 'react'
import { ListPlaceholder } from '@/components/ListPlaceholder'

type Recipient = { id:number; channel?:string|null; status:string; reason:string|null; sent_at:string|null; client:{id:string;full_name:string;phone:string|null;email:string|null}|null }
...
export default function MarketingPage() {
  ...
  async function send(campaign:Campaign,resume=false) {
    const segment = campaign.audience?.segment??'ALL'
    const aviso = resume
      ? `¿Retomar el envío de “${campaign.name}”? Solo se le escribirá a quienes todavía no la recibieron.`
      : `¿Enviar “${campaign.name}” por ${campaign.channel==='BOTH'?'WhatsApp y Email':campaign.channel} al segmento ${segment}? Solo entrarán clientes con consentimiento vigente.`
    if (!window.confirm(aviso)) return
    setSending(campaign.id)
    setError('')
    const response = await fetch('/api/admin/campaigns/send',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({campaignId:campaign.id,resume})})
  ...
  return <>
    ...
    <div className="grid gap-4 md:grid-cols-3"><article className="rounded-2xl border bg-white p-5"><UsersRound className="text-[#5b3df5]"/><p className="mt-4 text-sm text-[#736f83]">Campañas</p><b className="text-3xl">{campaigns.length}</b></article><article className="rounded-2xl border bg-white p-5"><MessageCircle className="text-[#5b3df5]"/><p className="mt-4 text-sm text-[#736f83]">Entregas WhatsApp</p><b className="text-3xl">{campaigns.flatMap((campaign)=>campaign.campaign_recipients.map((recipient)=>({...recipient,canal:recipient.channel??campaign.channel}))).filter((recipient)=>recipient.canal==='WHATSAPP'&&recipient.status==='SENT').length}</b></article><article className="rounded-2xl border bg-white p-5"><Mail className="text-[#5b3df5]"/><p className="mt-4 text-sm text-[#736f83]">Entregas Email</p><b className="text-3xl">{campaigns.flatMap((campaign)=>campaign.campaign_recipients.map((recipient)=>({...recipient,canal:recipient.channel??campaign.channel}))).filter((recipient)=>recipient.canal==='EMAIL'&&recipient.status==='SENT').length}</b></article></div>
    <div className="mt-6 rounded-2xl border bg-white p-5"><h2 className="font-extrabold">Campañas</h2><div className="mt-4 space-y-3">{campaigns.map((campaign)=>{const sent=campaign.campaign_recipients.filter((recipient)=>recipient.status==='SENT').length,failed=campaign.campaign_recipients.filter((recipient)=>recipient.status==='FAILED').length;return <div key={campaign.id} className="flex flex-col justify-between gap-3 rounded-xl border p-4 lg:flex-row lg:items-center"><div className="min-w-0"><b>{campaign.name}</b><p className="max-w-xl truncate text-sm text-[#736f83]">{campaign.channel==='BOTH'?'WhatsApp y Email':campaign.channel} · {campaign.content}</p><p className="mt-1 text-xs text-[#736f83]">{sent} entregadas{failed?` · ${failed} fallidas`:''}</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">{statusLabels[campaign.status]??campaign.status}</span><button title="Ver destinatarios" onClick={()=>setViewing(campaign)} className="rounded-lg border p-2"><Eye size={16}/></button>{campaign.status!=='SENDING'&&<button title="Editar" onClick={()=>setEditing(campaign)} className="rounded-lg border p-2"><Pencil size={16}/></button>}{['DRAFT','SCHEDULED','CANCELLED'].includes(campaign.status)&&<button onClick={()=>send(campaign)} disabled={sending===campaign.id} className="rounded-lg bg-[#5b3df5] px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{sending===campaign.id?'Enviando…':'Enviar'}</button>}{campaign.status==='SENDING'&&<button title="Solo se le escribe a quienes falten" onClick={()=>send(campaign,true)} disabled={sending===campaign.id} className="rounded-lg border border-[#5b3df5] px-3 py-2 text-xs font-bold text-[#5b3df5] disabled:opacity-50">{sending===campaign.id?'Retomando…':'Retomar envío'}</button>}</div></div>})}{campaigns.length===0&&<ListPlaceholder loading={loading} error={Boolean(error)} className="py-6 text-center text-sm text-[#736f83]">Aún no hay campañas.</ListPlaceholder>}</div></div>
  </>
}
