...
export async function POST(request:Request){
  try{
    ...
    const {data,error}=await db.rpc('create_safe_appointment',{p_business_id:businessId,p_branch_id:body.branchId??null,p_client_id:clientId,p_professional_id:body.professionalId,p_service_id:body.serviceId,p_desired_start:desiredStart.toISOString(),p_source:'CLIENT',p_notes:body.notes?.slice(0,1000)??null})
