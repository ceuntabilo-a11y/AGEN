...
export async function POST(request:Request){
  try{
    ...
    const {data,error}=await db.rpc('find_service_slots',{p_business_id:businessId,p_service_id:body.serviceId,p_from:from.toISOString(),p_until:until.toISOString(),p_interval_minutes:15,p_limit:50})
