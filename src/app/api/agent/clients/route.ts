...
export async function POST(request: Request) {
  ...
  if (lookupError) return NextResponse.json({ error: 'No se pudo consultar el cliente', motivo: 'ERROR_TECNICO' }, { status: 500 })
  ...
  if (existing) {
    ...
    return NextResponse.json({ created: false, motivo: 'OK', client: { ...existing, ...changes } })
  }

  if (!body.fullName?.trim()) return NextResponse.json({ error: 'Se necesita el nombre antes de registrar al cliente', needsName: true, motivo: 'DATO_INVALIDO' }, { status: 409 })
  ...
  if (error?.code === '23505') {
    ...
    return NextResponse.json({ created: false, motivo: 'OK', client: concurrent })
  }
  if (error) return NextResponse.json({ error: 'No se pudo registrar el cliente', motivo: 'ERROR_TECNICO' }, { status: 500 })
  ...
  return NextResponse.json({ created: true, motivo: 'OK', client }, { status: 201 })
}
