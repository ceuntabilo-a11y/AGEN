...
export async function POST(request: Request) {
  ...
  const { data, error } = await createAdminClient().rpc('find_service_slots', { p_business_id: body.businessId, p_service_id: body.serviceId, p_from: from.toISOString(), p_until: until.toISOString(), p_interval_minutes: 15, p_limit: 20 })
