...
export async function POST(request: Request) {
  ...
  if (!desiredStart || desiredStart.getTime() <= Date.now()) {
    return NextResponse.json({ error: 'desiredStart debe ser una fecha ISO futura válida', motivo: 'DATO_INVALIDO' }, { status: 400 })
  }

  if(await rejectTeamActor(supabase,body.businessId,body.actorPhone))return NextResponse.json({error:'El equipo solo puede consultar; usa el panel para gestionar reservas',motivo:'NO_AUTORIZADO'},{status:403})
  ...
  if(holdId){
    ...
    if(holdError)return NextResponse.json({error:'No se pudo validar el apartado',motivo:'ERROR_TECNICO'},{status:500})
    ...
    if(!hold||new Date(hold.expires_at).getTime()<=Date.now()){
      ...
      if(renovado.error||!renovado.data){
        ...
        return NextResponse.json({error:'Ese horario acaba de ocuparse',conflict:true,motivo:'CUPO_OCUPADO'},{status:409})
      }
      ...
      if(Math.abs(heldStart.getTime()-desiredStart.getTime())>1000)return NextResponse.json({error:'El horario no corresponde al apartado',conflict:true,motivo:'CUPO_OCUPADO'},{status:409})
    }
  }
  ...
  if (error?.code === '23P01') {
    return NextResponse.json({ error: error.message, conflict: true, motivo: motivoDeError(error) }, { status: 409 })
  }
  if (error) return NextResponse.json({ error: 'No se pudo crear la reserva', motivo: motivoDeError(error) }, { status: 500 })

  return NextResponse.json({ booked: true, motivo: 'OK', appointment: data }, { status: 201 })
}
